(function(){
  const VERSION='1.14.6',BUILD='1933';
  ...
  function bind(){const structure=document.getElementById('tripStructure');if(structure&&!structure.dataset.changeSemantics1933){structure.dataset.changeSemantics1933='1';structure.addEventListener('change',syncPlannerSemantics)}}
  ...
  function replaceImage(img,url,alt=''){if(!img)return;img.src=url;if(alt)img.alt=alt;delete img.dataset.reisslimFallback;img.decoding='async'}
  function patchDestinationCard(card,url,name){if(!card)return;const hero=card.querySelector('.proposal-hero');if(!hero)return;let img=hero.querySelector('img.destination-image');if(!img){img=createImage(url,`Beeld van ${name}`,{className:'destination-image'});const placeholder=hero.querySelector(':scope > span');if(placeholder)placeholder.replaceWith(img);else hero.prepend(img)}else replaceImage(img,url,`Beeld van ${name}`)}
  function patchQuickCard(card,url,name,index){if(!card)return;let img=card.querySelector(':scope > img');if(!img){const fallback=card.querySelector(':scope > .portfolio-fallback-photo');img=createImage(url,`Beeld van ${name}`,{eager:index<3,high:index<2});if(fallback)fallback.replaceWith(img);else card.prepend(img)}else replaceImage(img,url,`Beeld van ${name}`)}
  function patchThumb(scope,url){if(!scope)return;const img=scope.querySelector('img[data-reisslim-fallback="1"],.portfolio-thumb-loading');if(!img)return;if(img.tagName==='IMG')replaceImage(img,url);else{const className=[...img.classList].filter(c=>c!=='portfolio-thumb-loading').join(' ');img.replaceWith(createImage(url,'',{className}))}}
  function patchComparisonCard(card,url,name,index){if(!card)return;let img=card.querySelector(':scope > img');if(!img){img=createImage(url,`Beeld van ${name}`,{eager:index<2,high:index===0});card.prepend(img)}else replaceImage(img,url,`Beeld van ${name}`)}
  function patchPlanHero(url,name){const title=document.getElementById('planVisualTitle')?.textContent||'';if(!title||!String(title).toLocaleLowerCase('nl-NL').includes(String(name).toLocaleLowerCase('nl-NL')))return;const img=document.getElementById('planVisualImage');if(img){replaceImage(img,url,`Beeld van ${name} langs de voorgestelde route`);img.hidden=false}}
