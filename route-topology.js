...
export function buildAlternativeReturnNodes(origin,destination,legCount,{offsetKm=null}={}){
 ...
 const effectiveOffset=offsetKm??Math.max(75,Math.min(190,directKm*.16));
 const stops=[];
 // For a real loop we deliberately pull the return corridor farther away at mid-route.
 for(let index=1;index<=required;index++){const progress=index/legCount,direct=interpolateRoutePoint(origin,destination,progress,{}),wave=Math.sin(Math.PI*progress),offset=effectiveOffset*(.72+.28*wave);stops.push(shiftedPoint(direct,origin,destination,offset,progress,1))}
