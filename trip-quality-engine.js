...
export function calculateTripQuality(trip, destination, plan, budget) {
  const travelDays = plan.days.filter(day => ['outward', 'return'].includes(day.kind)).length;
  ...
  const dimensions = {
    ...
    realism: roundScore((plan.days.length === trip.days ? 35 : 0) + (plan.feasible ? 45 : 0) + (plan.routeMetrics.originKnown ? 20 : 5))
  };
