...
export function buildCanonicalTripPlan(trip,destination){
  ...
  days=normalizeDays(days,trip);const accommodationChanges=countAccommodationChanges(days,trip),oneWayTiming=timingForDistance(trip,direct),oneWayRoadHours=Number.isFinite(Number(metrics?.oneWayRoadHours))?Number(metrics.oneWayRoadHours):Number(oneWayTiming.roadHours||0),oneWayElapsedHours=Number.isFinite(Number(metrics?.oneWayElapsedHours))?Number(metrics.oneWayElapsedHours):Number(oneWayTiming.elapsedHours||0),oneWayBreakHours=Number.isFinite(Number(metrics?.breakHours))?Number(metrics.breakHours):Number(oneWayTiming.breakHours||0),plan={canonicalEngine:ENGINE,canonicalVersion:2,destinationId:destination?.id||null,days,routeMetrics:{origin:namedPoint(origin,trip.origin,{catalogId:'origin',role:'origin',landValidated:true}),oneWayDistanceKm:Math.round(direct),oneWayRoadHours,oneWayElapsedHours,oneWayDriveHours:oneWayElapsedHours,breakHours:oneWayBreakHours,requiredLegs:legs,maximumRoadLegKm:Math.round(maxLeg),routeSource:metrics?.routeSource||'canonical-v2',exploration:{overlap:null,explorationScore:100,method:'canonical-v2'}},requiredLegs:legs,usedLegs:legs,minimumDays,feasible:true,proposalCategory:destination?.category||'exact',warnings:[],accommodationChanges,routing:{source:'canonical-v2',label:trip.tripStructure==='base'?'Canonieke uitvalsbasisroute':openEnded?'Canonieke open-einde route':trip.routeTopology==='loop'?'Canonieke lusroadtrip':'Canonieke heen-en-terugroadtrip',live:false},origin:{name:trip.origin,...origin},topology:trip.routeTopology,baseSelection:trip.tripStructure==='base'?{point:{...anchor},label:`${anchor.name} als slimme uitvalsbasis`,why:{travelLegs:legs,minimumChanges}}:null};
...
export function canonicalPlanDiagnostic(trip,plan,destination=null){
  ...
  const requested=Math.max(1,Number(trip?.days||0)),rows=plan.days||[];
  const routeMetrics=plan?.routeMetrics||{};
  if(!Number.isFinite(Number(routeMetrics.oneWayDistanceKm))||!Number.isFinite(Number(routeMetrics.oneWayElapsedHours)))return{valid:false,code:'ui-contract-route-metrics',reason:'De route is inhoudelijk opgebouwd maar mist de afstand/reistijd-samenvatting die de Reisplan-weergave nodig heeft.',suggestion:'Herbouw het plan met de huidige canonieke engine; een incompleet plan mag nooit naar de gebruikersinterface worden gestuurd.'};
  if(rows.length!==requested)return{valid:false,code:'day-count',reason:`Het plan bevat ${rows.length} dagen terwijl ${requested} dagen zijn gevraagd.`,suggestion:'Bouw het plan opnieuw op; geen enkele fase mag kalenderdagen verwijderen.'};
