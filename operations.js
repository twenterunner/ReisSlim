...
import { assetHealth } from './advanced.js';
...
export function maintenanceSummary(data,horizonDays=90){const end=new Date(DEMO_NOW.getTime()+horizonDays*MS_DAY),policies=(data.maintenancePolicies||[]).map(p=>{const eq=data.equipment.find(e=>e.id===p.equipmentId),due=new Date(p.nextDueDate),days=Math.ceil((due-DEMO_NOW)/MS_DAY),event=(data.maintenance||[]).find(m=>m.policyId===p.id&&new Date(m.start)>=DEMO_NOW),health=assetHealth(data,p.equipmentId);return{...p,equipment:eq,days,event,health,dueWithin:due<=end,overdue:due<DEMO_NOW,predictiveDueWithin:health?.band==='High'&&new Date(DEMO_NOW.getTime()+health.predictedFailureWindowDays*MS_DAY)<=end}});return{policies,due:policies.filter(p=>p.dueWithin||p.predictiveDueWithin),overdue:policies.filter(p=>p.overdue),planned:policies.filter(p=>p.event),highRisk:policies.filter(p=>p.health?.band==='High'||(['High','Critical'].includes(p.criticality)&&(['High'].includes(p.failureRisk)||p.conditionScore<60))),plannedHours:Math.round(policies.reduce((a,p)=>a+(p.event?(new Date(p.event.end)-new Date(p.event.start))/3600000:0),0))};}
...
export function optimizeMaintenancePlan(data,horizonDays=90){ensureOperationsState(data);let planned=0,avoided=0,moved=0,predictive=0;const horizon=new Date(DEMO_NOW.getTime()+horizonDays*MS_DAY);const candidates=data.maintenancePolicies.filter(p=>{if(p.locked)return false;const h=assetHealth(data,p.equipmentId);return new Date(p.nextDueDate)<=horizon||(h?.band==='High'&&new Date(DEMO_NOW.getTime()+h.predictedFailureWindowDays*MS_DAY)<=horizon);});for(const p of candidates){const health=assetHealth(data,p.equipmentId),calendarDue=new Date(p.nextDueDate),predictiveDue=health?.band==='High'?new Date(DEMO_NOW.getTime()+health.predictedFailureWindowDays*MS_DAY):calendarDue,due=new Date(Math.min(calendarDue.getTime(),predictiveDue.getTime()));if(due<calendarDue)predictive++;const existing=data.maintenance.find(m=>m.policyId===p.id),calTarget=calibrationServiceTarget(data,p.equipmentId,due);const naiveStart=new Date(due);naiveStart.setUTCHours(8,0,0,0);const naiveEnd=new Date(naiveStart.getTime()+p.durationHours*3600000);const naiveConflicts=data.bookings.filter(b=>b.equipmentId===p.equipmentId&&b.status!=='Completed'&&overlap(naiveStart,naiveEnd,b.start,b.end)).length;let best=null;const lead=health?.band==='High'?45:30,earliest=new Date(Math.max(DEMO_NOW.getTime()+MS_DAY,due.getTime()-lead*MS_DAY));for(let t=weekStart(earliest).getTime();t<=due.getTime();t+=MS_DAY){const s=new Date(t);s.setUTCHours(8,0,0,0);if([0,6].includes(s.getUTCDay()))continue;const e=new Date(s.getTime()+p.durationHours*3600000);if(e>due||eventOverlap(data,p.equipmentId,s,e,p.id))continue;const conflicts=data.bookings.filter(b=>b.equipmentId===p.equipmentId&&b.status!=='Completed'&&overlap(s,e,b.start,b.end)).length,near=bookingHoursNear(data,p.equipmentId,s,3),calPenalty=calTarget?Math.abs(calTarget-s)/MS_DAY*.18:0,riskPenalty=(health?.risk||0)>=70?((s-DEMO_NOW)/MS_DAY)*.12:0,score=conflicts*1000+near+(due-s)/MS_DAY*.05+calPenalty+riskPenalty;if(!best||score<best.score)best={s,e,score,conflicts};}if(!best)continue;const pred=due<calendarDue,rec={id:existing?.id||`PMEV-${p.equipmentId}`,equipmentId:p.equipmentId,start:best.s.toISOString(),end:best.e.toISOString(),type:'Preventive Maintenance',reason:`Optimized ${p.strategy||'preventive'} maintenance${pred?' · advanced by predictive health risk':''}${calTarget&&Math.abs(calTarget-best.s)<=21*MS_DAY?' · aligned with calibration window':''}`,policyId:p.id,status:'Planned',optimized:true,predictiveTriggered:pred,healthRisk:health?.risk||0,calibrationAligned:!!(calTarget&&Math.abs(calTarget-best.s)<=21*MS_DAY)};if(existing){if(existing.start!==rec.start)moved++;Object.assign(existing,rec);}else{data.maintenance.push(rec);planned++;}avoided+=Math.max(0,naiveConflicts-best.conflicts);}data.settings.lastMaintenanceOptimization={timestamp:new Date().toISOString(),planned,moved,avoided,predictive};return{data,planned,moved,avoided,predictive};}
