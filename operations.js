...
export function inferOpportunityPlan(data,opp){
 ...
 const methods=ids.map(id=>data.methods.find(m=>m.id===id)).filter(Boolean);const staffHours=methods.reduce((s,m)=>s+(m.continuousStaffing?(n(m.setupHours)+n(m.executionHours)+n(m.teardownHours)+n(m.analysisHours)):(n(m.setupHours)+n(m.teardownHours)+n(m.analysisHours))),0);const equipmentHours=methods.reduce((s,m)=>s+n(m.setupHours)+n(m.executionHours)+n(m.teardownHours),0);return{methodIds:ids,methods,source,similarProgrammeId,similarity:Math.round(similarity*100),staffHours:Math.round(staffHours*10)/10,equipmentHours:Math.round(equipmentHours*10)/10};
}
...
function methodStaffHours(m){return m.continuousStaffing?n(m.setupHours)+n(m.executionHours)+n(m.teardownHours)+n(m.analysisHours):n(m.setupHours)+n(m.teardownHours)+n(m.analysisHours);}
...
export function capacityForecast(data,weeks=12,mode='weighted'){
 ...
 for(let wi=0;wi<weeks;wi++){
  ...
  for(const opp of data.opportunities||[]){if(opp.status==='Lost'||opp.probability<=0)continue;const start=new Date(opp.expectedStart),due=new Date(opp.dueDate);if(e<=start||s>=due)continue;const plan=inferOpportunityPlan(data,opp),activeWeeks=Math.max(1,Math.ceil((due-start)/(7*MS_DAY))),mult=opp.status==='Won'?1:demandMultiplier(mode,opp.probability);for(const m of plan.methods){const eh=(n(m.setupHours)+n(m.executionHours)+n(m.teardownHours))/activeWeeks*mult;if(eq[m.equipmentType])eq[m.equipmentType].pipeline+=eh;const sh=methodStaffHours(m)/activeWeeks*mult;for(const k of m.requiredSkills||[])if(sk[k])sk[k].pipeline+=sh;}}
