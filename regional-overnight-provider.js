import { mapConcurrent } from './roadtrip-runtime-engine.js?v=1931';
...
const CACHE_PREFIX='reisslim.regional-resilient.v4';
...
function overpassQuery(points){const clauses=points.map(p=>`nwr(around:30000,${Number(p.lat).toFixed(5)},${Number(p.lon).toFixed(5)})["place"~"^(city|town|village)$"]["name"];`).join('');return`[out:json][timeout:6][maxsize:8388608];(${clauses});out center tags 220;`}
...
function targetCandidateCount(trip){const days=Math.max(2,Number(trip?.days||5));return trip?.tripStructure==='base'?Math.max(5,Math.min(8,days)):Math.max(4,Math.min(7,Math.ceil(days/2)))}
function strategicSeeds(trip,origin,anchor,maxRequests){const rows=[],seen=[];const push=p=>{if(!finite(p)||seen.some(q=>geoKm(q,p)<8))return;seen.push(p);rows.push(p)};const limit=Math.max(8,maxRequests||20),base=trip?.tripStructure==='base';
 const centre=finite(anchor)?anchor:origin;
 if(finite(anchor))push(anchor);
 if(finite(anchor)&&geoKm(origin,anchor)>20){
   const bearing=initialBearing(origin,anchor),side=base?26:38;
   for(const f of [.16,.32,.50,.68,.84]){
     const p=lerp(origin,anchor,f);push(p);push(seed(p,side,bearing+90));push(seed(p,side,bearing-90));
   }
   for(const d of [70,135,205]){push(seed(origin,d,bearing));push(seed(origin,d,bearing+35));push(seed(origin,d,bearing-35))}
 }
 const radii=base?[28,52,78,105]:[45,80,120,165,215];
...
export function buildRegionalSeeds(trip,origin,anchor){return strategicSeeds(trip,origin,anchor,24)}
export async function discoverRegionalOvernightCandidates(trip,origin,anchor,{fetchImpl=globalThis.fetch,timeoutMs=5000,maxRequests=20,concurrency=8,onProgress,storage=globalThis.localStorage}={}){
 if(typeof fetchImpl!=='function'||!finite(origin))return[];
 const cacheKey=key(origin,anchor,trip),target=targetCandidateCount(trip),fresh=readCache(storage,cacheKey,FRESH_MS);
 // Two real regions are enough for a valid moving-trip fallback; a fresh cache
 // should therefore be useful immediately instead of forcing another network pass.
 if(fresh?.value?.length>=2){onProgress?.({completed:1,total:1,source:'cache',cached:true});return fresh.value}
 const stale=readCache(storage,cacheKey,STALE_MS);
 const seeds=strategicSeeds(trip,origin,anchor,maxRequests);
 let completed=0;

 // One batched Overpass request usually returns many named towns at once and is
 // substantially faster than reverse-geocoding every seed first.
 onProgress?.({completed:0,total:1,source:'overpass',stage:'batch'});
 let found=dedupe(await overpassBatch(seeds,fetchImpl,Math.min(3400,timeoutMs)),origin);
 onProgress?.({completed:1,total:1,source:'overpass',stage:'batch'});

 if(found.length<target){
   const prioritySeeds=seeds.filter(s=>!found.some(x=>geoKm(x,s)<32)).slice(0,Math.min(12,Math.max(6,target*2)));
   const photon=await mapConcurrent(prioritySeeds,async(point,index)=>{const row=await photonReverse(point,fetchImpl,Math.min(2400,timeoutMs));completed++;onProgress?.({completed,total:prioritySeeds.length,source:'photon',index});return row},{concurrency:Math.min(8,Math.max(3,concurrency||8))});
   found=dedupe([...found,...photon.map(x=>x?.error?null:x).filter(Boolean)],origin);
 }

 // Merge a recent stale cache before using the deliberately rate-limited public
 // Nominatim fallback. This makes intermittent provider outages cheap and fast.
 if(stale?.value?.length){
   const cachedPoints=stale.value.map(item=>{const b=item?.bases?.[0];return finite(b)?{name:b.name||item.name.replace(/\s*&\s*omgeving$/i,''),lat:Number(b.lat),lon:Number(b.lon),country:item.country||'Live regio',source:'cache'}:null}).filter(Boolean);
   found=dedupe([...found,...cachedPoints],origin);
 }

 if(found.length<2){
   const missing=seeds.filter(s=>!found.some(x=>geoKm(x,s)<35)).slice(0,2);
   for(let i=0;i<missing.length;i++){
     if(i)await wait(1050);
     const row=await nominatimReverse(missing[i],fetchImpl,Math.min(2600,timeoutMs));
 ...
 const profiles=found.slice(0,24).map((p,i)=>profile(p,i,origin,found,trip,p.source==='cache'));
