...
const FIELD_IDS = [
  'tripName','origin','startDate','days','budget','adults','children','transport','travelMode','routeTopology','tripPace',
  'destinationQuery','routeStyle','fuelRangeKm','vehicleMaxSpeedKmh','vehicleHeightM','vehicleLengthM','vehicleWeightKg',
...
export function resolveOrigin(originOrTrip){if(originOrTrip&&typeof originOrTrip==='object'&&validCoordinate(originOrTrip.originPoint))return{...originOrTrip.originPoint,name:originOrTrip.origin};const origin=typeof originOrTrip==='object'?originOrTrip.origin:originOrTrip;const key=String(origin||'').trim().toLocaleLowerCase('nl-NL');return originCatalog[key]?{...originCatalog[key]}:null}
...
export function normalizeTrip(input={}){
  ...
  // v1.4.3: temporarily roadtrip-only. Ignore historic fly/rail values from saved drafts.
  const travelMode='direct';
  const transport=transportId(input.transport);
  const spec=vehicleSpec({...input,transport});
  return{
    id:input.id||uniqueId(),tripName:String(input.tripName||'').trim().slice(0,60),origin:String(input.origin||'').trim(),
    ...
    startDate:String(input.startDate||''),days:Number(input.days),budget:Number(input.budget),adults:Number(input.adults),children:Number(input.children||0),
    transport,travelMode,routeTopology:['loop','out-and-back'].includes(input.routeTopology)?input.routeTopology:'loop',
    ...
    routeStyle:spec.routeStyle,fuelRangeKm:spec.fuelRangeKm,vehicleMaxSpeedKmh:spec.maxSpeedKmh,vehicleHeightM:spec.heightM,vehicleLengthM:spec.lengthM,vehicleWeightKg:spec.weightKg,
    maxDrive:Number(input.maxDrive),maxChanges:Number(input.maxChanges),comfort:['budget','mid','comfort'].includes(input.comfort)?input.comfort:'mid',
    strictBudget:input.strictBudget!==false&&input.strictBudget!=='false',strictDrive:input.strictDrive!==false&&input.strictDrive!=='false',
    strictChanges:input.strictChanges!==false&&input.strictChanges!=='false',allowStretch:input.allowStretch!==false&&input.allowStretch!=='false',
    liveData:input.liveData!==false&&input.liveData!=='false',remoteTravel:input.remoteTravel===true||input.remoteTravel==='true',
    privateMode:input.privateMode===true||input.privateMode==='true',notes:String(input.notes||'').trim().slice(0,500),
    preferences,preferenceWeights:weights,updatedAt:input.updatedAt||new Date().toISOString()
  }
}

export function validateTripInput(trip){
  const errors=[];if(!trip.origin)errors.push('Vul een vertrekplaats in.');
...
export function readTripForm(existing=null,root=document){
  const form=getFormElements(root),existingTrip=existing&&typeof existing==='object'?existing:null;
  ...
  return normalizeTrip({id:existingTrip?.id||existing||undefined,...values,travelMode:'direct',
...
export function writeTripForm(trip={},root=document){
  const form=getFormElements(root);for(const[key,element]of Object.entries(form)){if(!element||trip[key]===undefined||trip[key]===null)continue;if(element.type==='checkbox')element.checked=Boolean(trip[key]);else element.value=String(trip[key])}
  root.querySelectorAll('[data-pref]').forEach(box=>{box.checked=trip.preferences?.includes(box.value)||false});
  root.querySelectorAll('[data-priority]').forEach(select=>{const selected=root.querySelector(`[data-pref][value="${select.dataset.priority}"]`)?.checked;select.value=String(trip.preferenceWeights?.[select.dataset.priority]||2);select.disabled=!selected});
}
