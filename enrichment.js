...
export const liveProviders={
  ...
  async accommodation(night,plan){
  ...
  async images(plan){
    const labels=[plan.destinationName,plan.destinationAnchorName,plan.destinationCountry].filter(Boolean),query=encodeURIComponent(`${labels.join(' ')} landscape`);const d=await jsonFetch(`https://commons.wikimedia.org/w/api.php?action=query&generator=search&gsrsearch=${query}&gsrnamespace=6&gsrlimit=10&prop=imageinfo&iiprop=url|extmetadata&iiurlwidth=1200&format=json&origin=*`,providerConfig.imageTimeoutMs);const identityTokens=[plan.destinationName,plan.destinationAnchorName].filter(Boolean).map(x=>norm(x).split(' ')[0]).filter(x=>x.length>=3);for(const page of Object.values(d.query?.pages||{})){const info=page.imageinfo?.[0],text=norm(`${page.title} ${info?.extmetadata?.ImageDescription?.value||''}`);if(info?.thumburl&&identityTokens.some(token=>text.includes(token)))return{url:info.thumburl,verifiedEntityId:plan.destinationId,source:'Wikimedia Commons',sourceUrl:info.descriptionurl||'https://commons.wikimedia.org/',title:page.title}}return null;
  }
};
