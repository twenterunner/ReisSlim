...
export function renderItineraryVariants(state) {
  ...
  $('variantCards').innerHTML = variants.map(variant => `<article class="variant-card ${variant.selectable ? '' : 'not-feasible'}"><p class="eyebrow">${escapeHtml(variant.label)}</p><h3>${escapeHtml(state.destination?.name || '')}</h3><p>${escapeHtml(variant.summary)}</p><div class="variant-metrics"><span>± €${variant.metrics.total.toLocaleString('nl-NL')}</span><span>max ${variant.metrics.maxDrive.toFixed(1)} u</span><span>${variant.metrics.changes} wissels</span><span>plankwaliteit ${variant.metrics.quality}/100</span></div><div class="constraint-summary ${variant.constraintStatus.category}">${escapeHtml(variant.constraintStatus.summary)}</div>${variant.qualityWarning ? `<div class="inline-warning">${escapeHtml(variant.qualityWarning)}</div>` : ''}<button type="button" data-select-variant="${variant.id}" ${variant.selectable ? '' : 'disabled'}>${variant.selectable ? `Kies ${escapeHtml(variant.label.toLowerCase())}` : 'Planstructuur onvoldoende'}</button></article>`).join('');
}
...
export function renderOptimizationPreview(state) {
  ...
  preview.innerHTML = `<div class="optimizer-verdict ${proposal.meaningful ? 'ok' : 'warn'}"><strong>${before}/100 → ${after}/100</strong><p>${escapeHtml(proposal.message)}</p><small>${escapeHtml(proposal.threshold)}</small></div>${proposal.actions.length ? `<fieldset><legend>Selecteer structurele wijzigingen</legend>${proposal.actions.map(action => { const change = proposal.changeSet?.find(item => item.actionId === action.id); return `<label class="optimizer-action"><input type="checkbox" data-optimization-action="${action.id}" checked><span><strong>${escapeHtml(action.title)}</strong><small>${escapeHtml(action.description)}</small><small>Betrokken dagen: ${escapeHtml((change?.affectedDays || action.affectedDays || []).join(', ') || 'geen')}</small></span></label>`; }).join('')}</fieldset>` : ''}`;
}
...
function renderDayRecommendations(day) {
  ...
  return `<div class="day-recommendations"><h5>Waar stoppen, slapen, eten en wat te zien</h5><div class="recommendation-grid">${day.recommendations.map(item => {
    ...
    const sourceLicense = item.license ? ` · licentie: ${escapeHtml(item.license)}` : '';
    return `<article class="place-proposal ${escapeHtml(item.type)}"><span>${escapeHtml(recommendationLabels[item.type] || item.type)}</span><strong>${escapeHtml(item.name)}</strong><p>${escapeHtml(item.reason)}</p>${item.associatedBase ? `<small>Basis: ${escapeHtml(item.associatedBase)}</small>` : ''}${item.openingHours ? `<small>Bron vermeldt: ${escapeHtml(item.openingHours)} · niet opnieuw geverifieerd</small>` : ''}${Number.isFinite(item.straightLineDistanceKm) ? `<small>Hemelsbrede afstand tot planpunt: ± ${item.straightLineDistanceKm.toFixed(1)} km · echte omweg onbekend</small>` : ''}<small>${escapeHtml(item.source)}${sourceLicense} · ${item.genericFallback ? 'duidelijk categorievoorstel' : 'genoemde providerlocatie'} · vertrouwen: ${escapeHtml(item.confidence || 'onbekend')}</small>${item.availabilityWarning ? `<small class="recommendation-warning">${escapeHtml(item.availabilityWarning)}</small>` : ''}${sourceUrl ? `<a href="${escapeHtml(sourceUrl)}" target="_blank" rel="noopener noreferrer">Bekijk bron</a>` : ''}</article>`;
  }).join('')}</div></div>`;
}
...
export function renderPlan(state) {
  ...
  $('budgetAssumptions').innerHTML = `<summary>Aannames en vertrouwen</summary><p>Ramingvertrouwen: <strong>${escapeHtml(budget.confidence)}</strong>. Brandstof €${budget.assumptions.fuelPricePerLitre.toFixed(2)}/l, verbruik ${budget.assumptions.consumption} l/100 km, overnachtingsprior €${Number(budget.assumptions.nightlyRate || 0).toFixed(0)}/nacht, activiteitenprior €${Number(budget.assumptions.activityRatePerDay || 0).toFixed(0)}/dag en onvoorzien ${Math.round(budget.assumptions.contingencyRate * 100)}%. Overnachtingstype: ${escapeHtml(profile.accommodationLabel)}. ${escapeHtml(budget.assumptions.costModelSource)}. Afstand: ${escapeHtml(budget.assumptions.roadDistanceSource)}. Prijzen, beschikbaarheid en lokale prijsniveaus zijn niet live geverifieerd.</p>`;
  ...
  const labels = quality.dimensionLabels || {};
  ...
  $('qualityDimensions').innerHTML = Object.entries(quality.dimensions).map(([key, value]) => `<div class="quality-row"><span>${escapeHtml(labels[key] || key)}</span><div class="quality-bar"><i style="width:${value}%"></i></div><strong>${value}/100</strong></div>`).join('');
  ...
  $('mapDataStatus').textContent = `${plan.routing?.label || 'Offline corridorraming'} · ${plan.placeData?.live ? `${plan.placeData.namedPlaces} live plaatsen` : plan.placeData?.catalogue ? `${plan.placeData.namedPlaces || 0} genoemde catalogusplaatsen` : 'offline plaatsen'}`;
