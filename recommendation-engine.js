...
import { transportId, vehicleMatchLabel } from './vehicle-intelligence.js';
  ...
  for (const day of days) {
    ...
    if (!isHomecoming) {
      recommendations.push(proposal({
        day: day.day,
        type: 'accommodation',
        name: `Zoek ${vehicleMatchLabel(trip)} in of nabij ${day.overnight}`,
        reason: isTravel
      ...
      recommendations.push(proposal({
        day: day.day,
        type: 'restaurant',
        name: `${rule.restaurant} in ${day.location}`,
        reason: isTravel
