...
function pointAt(day,f=.5){const g=path(day);return g.length?g[Math.min(g.length-1,Math.max(0,Math.round(f*(g.length-1))))]:null}
function cacheKey(type,p){return`${PREFIX}:${type}:${Number(p.lat).toFixed(2)},${Number(p.lon).toFixed(2)}`}
function readCache(storage,k,maxAge){try{const r=JSON.parse(storage?.getItem(k)||'null');return r?.savedAt&&Date.now()-r.savedAt<=maxAge?r.value:null}catch{return null}}
function writeCache(storage,k,v){try{storage?.setItem(k,JSON.stringify({savedAt:Date.now(),value:v}))}catch{}}
...
function normOverpass(e,type,target,day){const tags=e.tags||{},point={lat:Number(e.lat??e.center?.lat),lon:Number(e.lon??e.center?.lon)};let name=esc(tags.name||tags['name:nl']||tags['name:en']||tags.brand);if(!valid(point))return null;if(type==='rest'&&!name&&['rest_area','services'].includes(tags.highway))name=tags.highway==='services'?'Serviceplaats':'Rustplaats';if(!name)return null;const d=km(target,point);return{type,name,point,lat:point.lat,lon:point.lon,day:day.day,live:true,genericFallback:false,verified:false,source:'OpenStreetMap Overpass',detourKm:Number(d.toFixed(1)),reason:`Specifieke ${type} voor dag ${day.day}, dicht bij de geplande route.`,mapUrl:`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(name+' '+point.lat+','+point.lon)}`}}
async function overpassResolve(day,target,type,fetchImpl,timeoutMs){const q=`[out:json][timeout:6][maxsize:4194304];(${block(target,type)});out center tags 100;`,body=new URLSearchParams({data:q});return await new Promise(resolve=>{let left=ENDPOINTS.length,done=false;for(const ep of ENDPOINTS){fetchJson(ep,{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded',accept:'application/json'},body},fetchImpl,timeoutMs).then(payload=>{if(done)return;const rows=(payload?.elements||[]).map(e=>normOverpass(e,type,target,day)).filter(Boolean).sort((a,b)=>a.detourKm-b.detourKm);if(rows[0]){done=true;resolve(rows[0]);return}if(--left===0){done=true;resolve(null)}}).catch(()=>{if(--left===0&&!done){done=true;resolve(null)}})}})}
