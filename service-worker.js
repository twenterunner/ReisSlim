...
const CACHE='reisslim-v1.15.3-build-1943-runtime-contract';
const IMAGE_CACHE='reisslim-runtime-images-v3';
const RELEASE_VERSION='1.15.3';
const RELEASE_BUILD='1943';
const ASSETS=[
...
'./roadtrip-runtime-engine.js','./start-new-trip.js','./planner-submit-guard.js','./route-stop-provider-1929.js','./overnight-accommodation-1929.js','./runtime-source-repair-1943.js','./release-sync-1943.js'
];
...
self.addEventListener('activate',event=>{
  event.waitUntil((async()=>{
    ...
    // Reload every open ReisSlim window exactly once so 1943 takes control of
    // app.js immediately instead of requiring a second manual refresh/restart.
    const clients=await self.clients.matchAll({type:'window',includeUncontrolled:true});
    await Promise.allSettled(clients.map(async client=>{
      try{
        const url=new URL(client.url);
        if(url.origin!==self.location.origin||url.searchParams.get('runtime')==='1943')return;
        url.searchParams.set('runtime','1943');
...
function syncSourceText(text,pathname=''){
  let out=String(text)
  .replace(/1\.13\.0-1923-data-engine/g,'1.15.3-1943-runtime-contract')
  .replace(/1\.14\.1-1928-synchronized/g,'1.15.3-1943-runtime-contract')
  .replace(/1\.14\.2-1929-(?:complete-stops|header-repair)/g,'1.15.3-1943-runtime-contract')
  .replace(/1\.14\.3-1930-roadtrip-policy/g,'1.15.3-1943-runtime-contract')
  .replace(/1\.14\.4-1931-roadtrip-resilience/g,'1.15.3-1943-runtime-contract')
  .replace(/1\.14\.5-1932-topology-supply/g,'1.15.3-1943-runtime-contract')
  .replace(/1\.14\.6-1933-(?:cape-global-images|global-images)/g,'1.15.3-1943-runtime-contract')
  .replace(/1\.15\.0-1940-global-adaptive/g,'1.15.3-1943-runtime-contract')
  .replace(/1\.15\.1-1941-selected-trip-contract/g,'1.15.3-1943-runtime-contract')
  .replace(/1\.15\.2-1942-bounded-solver/g,'1.15.3-1943-runtime-contract')
  .replace(/\?v=(?:1923|1928|1929|1930|1931|1932|1933|1940|1941|1942)\b/g,'?v=1943')
  .replace(/version:'1\.13\.0',build:'1923'/g,"version:'1.15.3',build:'1943'")
  .replace(/version:'1\.14\.1',build:'1928'/g,"version:'1.15.3',build:'1943'")
  .replace(/version:'1\.14\.2',build:'1929'/g,"version:'1.15.3',build:'1943'")
  .replace(/version:'1\.14\.3',build:'1930'/g,"version:'1.15.3',build:'1943'")
  .replace(/version:'1\.14\.4',build:'1931'/g,"version:'1.15.3',build:'1943'")
  .replace(/version:'1\.14\.5',build:'1932'/g,"version:'1.15.3',build:'1943'")
  .replace(/version:'1\.15\.0',build:'1940'/g,"version:'1.15.3',build:'1943'")
  .replace(/version:'1\.15\.1',build:'1941'/g,"version:'1.15.3',build:'1943'")
  .replace(/version:'1\.15\.2',build:'1942'/g,"version:'1.15.3',build:'1943'")
  .replace(/\.\/poi-gap-filler\.js\?v=1943/g,'./route-stop-provider-1929.js?v=1943')
  .replace(/\.\/overnight-accommodation\.js\?v=1943/g,'./overnight-accommodation-1929.js?v=1943');
...
function injectReleaseScript(html){
  let text=syncSourceText(html)
    .replace(/<script[^>]*release-sync-(?:1928|1929|1930|1931|1932|1933|1940|1941|1942|1943)\.js[^>]*><\/script>/gi,'');
  ...
  return text.replace(/<\/body>/i,'<script src="./release-sync-1943.js?v=1943"></script></body>')
}
