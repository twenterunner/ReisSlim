const CACHE = 'reisslim-v1.4.1-build-1401';
const ASSETS = [
  ...
  './assistant-engine.js', './weather-engine.js', './image-provider.js', './map-view.js', './gpx-generator.js', './ui-renderer.js', './ui-feature-flags.js'
];
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys().then(keys => Promise.all(
      keys.filter(key => key.startsWith('reisslim-') && key !== CACHE).map(key => caches.delete(key))
    ))
  );
  self.clients.claim();
});
...
async function networkFirst(request) {
  try {
    // cache:'reload' bypasses the browser HTTP cache. This matters because
    // index.html in older deployments still references app.js?v=1000.
...
self.addEventListener('fetch', event => {
  ...
  // IMPORTANT: scripts, modules, CSS and the service worker's config must be
  // network-first. Cache-first here was the reason an uploaded release could
  // still look like the old app.
  const destination = event.request.destination;
  const isCode = ['script', 'style', 'worker'].includes(destination)
