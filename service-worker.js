const CACHE = 'reisslim-v1.4.0-build-1400';
const ASSETS = [
  './', './index.html', './styles.css?v=1400', './app.js?v=1400', './manifest.webmanifest', './icon.svg',
...
self.addEventListener('fetch', event => {
  ...
  if (event.request.mode === 'navigate') { event.respondWith(fetch(event.request).then(response => response.ok ? response : Promise.reject(new Error('navigation failed'))).catch(() => caches.match('./index.html'))); return; }
