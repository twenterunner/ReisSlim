...
test('New implementation module is cached for offline PWA use and release version is consistent',()=>{const sw=fs.readFileSync('./service-worker.js','utf8'),pkg=JSON.parse(fs.readFileSync('./package.json','utf8')),data=fs.readFileSync('./data.js','utf8');assert.ok(sw.includes("labos-v1.11.0"));assert.ok(sw.includes('./implementation-governance.js'));assert.equal(pkg.version,'1.11.0');assert.ok(data.includes("APP_VERSION = '1.11.0'"));});
